module.exports = (context) => ({
  ...
  processAfterEach: ({ data, context }) => {
    let updateData = Object.entries(context.input).reduce(
      (result, [key, value]) => result.replace(new RegExp(`__${key}__`, 'g'), value),
      data
    );
    updateData = updateData.replace(/\$camelCaseComponentName\$/g, context._toCamelCase(context.input.componentName));
