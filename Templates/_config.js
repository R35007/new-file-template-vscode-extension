...
module.exports = () => ({
  ...
  input: {
    tags: {
      ...
      options: [
        { label: 'React', description: 'React', detail: 'This is a React Component', value: 'react' },
    ...
    componentName: {
      when: (config) => config.inputValues.tags?.includes('react'), // prompts only when tags includes 'react'
      value: 'AppComponent',
      promptAlways: false,
      password: false,
      ignoreFocusOut: false,
      ...
      transform: (value, config) => config._toPascalCase(value) // always transform takes the precedence
